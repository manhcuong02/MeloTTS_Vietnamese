...
def split_sentences_vi(
    raw_text: str,
    max_sentences: int = 1,
    max_words=30,
) -> List[str]:
    """
    Chunk the input text into smaller pieces, each containing
    at most `max_sentences` sentences.

    :param raw_text: The input text.
    :param max_sentences: The maximum number of sentences in each chunk.
    :return: A list of chunks.
    """
    raw_text = raw_text.replace("\n", ". ")
    sentences = re.split(
        r"(?<!\d\.)(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?|!)\s", raw_text
    )
    chunks = []
    current_chunk = []
    for sentence in sentences:
        ...
        # Split the sentence into smaller parts if it is too long
        if len(sentence) > 1:
            ...
            for p in parts:
                merged_part = part + " " + p
                if len(merged_part.split()) > max_words:
                    if len(part):
                        split_sentence.append(
                            part.strip(string.punctuation + string.whitespace)
                        )
                        part = p
                    else:
                        split_sentence.append(
                            p.strip(string.punctuation + string.whitespace)
                        )
                        part = ""
                else:
                    part += ", " + p
        ...
        # If the sentence is less than the max_sentences, add it to the current_chunk
